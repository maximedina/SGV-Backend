...
namespace MOM.Core.Util.Formula
{
    ...
    public class Automaton
    {
        ...
        public Automaton(string formula)
        {
            ...
            this.v_aux = "";
            this.n_aux = "";
            this._formulaExpression = formula;
        }
        ...
        {
            this.currentState = STATES.S;
            _formulaExpression = _formulaExpression.Replace(" ", "");
            bracketStack.Clear();
            executionQueue.Clear();
            operatorStack.Clear();
            variables.Clear();
            ...
            try
            {
                for (int i = 0; i < _formulaExpression.Length; i++)
                    this.changeState(_formulaExpression, i);
        ...
        public double evaluate(System.Collections.Hashtable valuesByVariables, int precision = 8)
        {
            if (!validateSyntax())
            {
                throw new Exception("A expressão instanciada não é válida");
            }
        ...
        private void doOperation(Element element, ref Stack<double> calculationStack)
        {
            ...
            {
                case TipoOperador.ABS:
                    calculationStack.Push(Math.Abs(calculationStack.Pop() + calculationStack.Pop()));
                    break;
